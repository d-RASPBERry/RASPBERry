...
################################################################################
# SAC 4-GPU 并行实验脚本 (BipedalWalker)
# 
# 实验计划:
#   GPU 0-3: 全部运行 BipedalWalker-v3 (向量环境, 24维状态 → 4维动作)
#
# 每个GPU运行1组实验(2个算法):
#   - SAC-PER (基线)
#   - SAC-RASPBERry (压缩版本)
...
#
# 使用方法:
#   ./run_sac_4gpu_parallel_BipedalWalker.sh
#
################################################################################
...
echo "================================================================================"
echo "🚀 启动 SAC 4-GPU 并行实验 (BipedalWalker)"
echo "================================================================================"
...
for gpu in "${GPU_IDS[@]}"; do
    replicate=$((gpu + 1))
    header="┌────────────────────────────────────────────────────────────────────────────┐"
    title=$(printf "│ GPU %d: BipedalWalker-v3 (重复实验 #%d)                                 │" "${gpu}" "${replicate}")
    footer="└────────────────────────────────────────────────────────────────────────────┘"
    echo "${header}"
    echo "${title}"
    echo "${footer}"
    ...
    echo "  [1/2] 启动 SAC-PER-BipedalWalker (GPU ${gpu})..."
    ...
    pid_per=$!
    ALL_PIDS+=($pid_per)
    ALL_NAMES+=("GPU${gpu}-SAC-PER-BipedalWalker")
    echo "       PID: ${pid_per}"
    echo "       配置: ${PER_CONFIG}"
    echo "       ⏳ 等待 ${LAUNCH_DELAY_SAME_GPU} 秒后启动 RASPBERry..."
    sleep ${LAUNCH_DELAY_SAME_GPU}

    echo "  [2/2] 启动 SAC-RASPBERry-BipedalWalker (GPU ${gpu})..."
    ...
    pid_rasp=$!
    ALL_PIDS+=($pid_rasp)
    ALL_NAMES+=("GPU${gpu}-SAC-RASPBERry-BipedalWalker")
    echo "       PID: ${pid_rasp}"
    echo "       配置: ${RASP_CONFIG}"
...
for idx in "${!GPU_IDS[@]}"; do
    gpu=${GPU_IDS[$idx]}
    per_name="GPU${gpu}-SAC-PER-BipedalWalker"
    rasp_name="GPU${gpu}-SAC-RASPBERry-BipedalWalker"
