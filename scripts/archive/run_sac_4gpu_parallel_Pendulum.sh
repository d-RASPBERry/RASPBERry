...
################################################################################
# SAC 4-GPU 并行实验脚本 (Pendulum)
# 
# 实验计划:
#   GPU 0-3: 全部运行 Pendulum-Pendulum (图像环境, 84×84×3, CNN)
#
# 每个GPU运行1组实验(2个算法):
#   - SAC-PER (基线)
#   - SAC-RASPBERry (压缩版本)
...
#
# 使用方法:
#   ./run_sac_4gpu_parallel_Pendulum.sh
#
################################################################################
...
echo "================================================================================"
echo "🚀 启动 SAC 4-GPU 并行实验 (Pendulum)"
echo "================================================================================"
...
echo ""
echo "实验配置:"
echo "  环境: Pendulum-Pendulum (84×84×3图像, CNN)"
echo "  GPU: 0-3 (4个重复实验)"
echo "  算法: SAC-PER + SAC-RASPBERry"
echo ""
echo "配置文件:"
echo "  PER:       configs/experiments/sac/per/pendulum.yml"
echo "  RASPBERry: configs/experiments/sac/raspberry/pendulum.yml"
echo ""
...
LAUNCH_DELAY_BETWEEN_GPUS=15  # GPU组之间间隔15秒 (CNN初始化慢)
LAUNCH_DELAY_SAME_GPU=10      # 同一GPU上两个实验间隔10秒 (图像环境)

for gpu in "${GPU_IDS[@]}"; do
    replicate=$((gpu + 1))
    header="┌────────────────────────────────────────────────────────────────────────────┐"
    title=$(printf "│ GPU %d: Pendulum (重复实验 #%d)                                          │" "${gpu}" "${replicate}")
    footer="└────────────────────────────────────────────────────────────────────────────┘"
    echo "${header}"
    echo "${title}"
    echo "${footer}"
    ...
    echo "  [1/2] 启动 SAC-PER-Pendulum (GPU ${gpu})..."
    ...
    pid_per=$!
    ALL_PIDS+=($pid_per)
    ALL_NAMES+=("GPU${gpu}-SAC-PER-Pendulum")
    echo "       PID: ${pid_per}"
    echo "       配置: ${PER_CONFIG}"
    echo "       ⏳ 等待 ${LAUNCH_DELAY_SAME_GPU} 秒后启动 RASPBERry..."
    sleep ${LAUNCH_DELAY_SAME_GPU}

    echo "  [2/2] 启动 SAC-RASPBERry-Pendulum (GPU ${gpu})..."
    ...
    pid_rasp=$!
    ALL_PIDS+=($pid_rasp)
    ALL_NAMES+=("GPU${gpu}-SAC-RASPBERry-Pendulum")
    echo "       PID: ${pid_rasp}"
    echo "       配置: ${RASP_CONFIG}"
...
for idx in "${!GPU_IDS[@]}"; do
    gpu=${GPU_IDS[$idx]}
    per_name="GPU${gpu}-SAC-PER-Pendulum"
    rasp_name="GPU${gpu}-SAC-RASPBERry-Pendulum"
