...
#
# 使用方法:
#   ./run_sac_ablation_BipedalWalker.sh                # 默认 GPU 共享 (仅使用 GPU 0)
#   ./run_sac_ablation_BipedalWalker.sh -n 0,1,2       # 指定逗号分隔 GPU 列表
#   ./run_sac_ablation_BipedalWalker.sh -m exclusive   # 开启独占模式 (需提供3的倍数GPU)
#
################################################################################
...
if [ "${GPU_ASSIGNMENT_MODE}" = "shared" ]; then
    ...
    for ((group_idx=0; group_idx<GROUP_COUNT; group_idx++)); do
        ...
        echo "┌─ 第 $((group_idx + 1)) 组 BipedalWalker SAC 消融任务 (独占模式) ───────────────┐"
        log_suffix="bipedalwalker_group$((group_idx + 1))_${TIMESTAMP}"
