...
################################################################################
# DDQN ablation launcher (MiniGrid-DoorKey-8x8)
################################################################################
...
PER_CONFIG="configs/experiments/ddqn/per/minigrid_doorkey_8x8.yml"
PBER_CONFIG="configs/experiments/ddqn/pber/minigrid_doorkey_8x8.yml"
RASP_CONFIG="configs/experiments/ddqn/raspberry/minigrid_doorkey_8x8.yml"
for cfg in "${PER_CONFIG}" "${PBER_CONFIG}" "${RASP_CONFIG}"; do
    [ -f "${cfg}" ] || { echo "Error: missing config ${cfg}" >&2; exit 1; }
done

echo "================================================================================"
echo "DDQN ablation (MiniGrid-DoorKey-8x8)"
...
if [ "${GPU_ASSIGNMENT_MODE}" = "shared" ]; then
    for idx in "${!GPU_IDS[@]}"; do
        gpu="${GPU_IDS[$idx]}"
        log_suffix="minigrid_doorkey_8x8_gpu${gpu}_${TIMESTAMP}"
    ...
    for ((group_idx=0; group_idx<GROUP_COUNT; group_idx++)); do
        ...
        log_suffix="minigrid_doorkey_8x8_group$((group_idx + 1))_${TIMESTAMP}"
