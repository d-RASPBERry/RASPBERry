...
PER_CONFIG="configs/experiments/sac/per/halfcheetah.yml"
PBER_CONFIG="configs/experiments/sac/pber/halfcheetah.yml"
RASP_CONFIG="configs/experiments/sac/raspberry/halfcheetah.yml"

if [ "${GPU_ASSIGNMENT_MODE}" = "shared" ]; then
    for idx in "${!GPU_IDS[@]}"; do
        gpu="${GPU_IDS[$idx]}"
        echo "┌─ GPU ${gpu}: 第 $((idx+1)) 组 HalfCheetah SAC 消融任务 ───────────────────┐"
        log_suffix="halfcheetah_gpu${gpu}_${TIMESTAMP}"
    ...
    for ((group_idx=0; group_idx<GROUP_COUNT; group_idx++)); do
        ...
        echo "┌─ 第 $((group_idx + 1)) 组 HalfCheetah SAC 消融任务 (独占模式) ───────────────┐"
        log_suffix="halfcheetah_group$((group_idx + 1))_${TIMESTAMP}"
