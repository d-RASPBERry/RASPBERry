...
if [ "${GPU_ASSIGNMENT_MODE}" = "shared" ]; then
    for idx in "${!GPU_IDS[@]}"; do
        gpu="${GPU_IDS[$idx]}"
        echo "┌─ GPU ${gpu}: 第 $((idx+1)) 组 HalfCheetah-Image SAC 消融任务 ─────────────┐"
        log_suffix="halfcheetah_image_gpu${gpu}_${TIMESTAMP}"
    ...
    for ((group_idx=0; group_idx<GROUP_COUNT; group_idx++)); do
        ...
        echo "┌─ 第 $((group_idx + 1)) 组 HalfCheetah-Image SAC 消融任务 (独占模式) ─────────┐"
        log_suffix="halfcheetah_image_group$((group_idx + 1))_${TIMESTAMP}"
