...
################################################################################
# APEX/DDQN test launcher (Pong) - PBER + RASPBERry
#
# Runs 4 variants per GPU:
#   1) APEX-PBER
#   2) APEX-RASPBERry
#   3) DDQN-PBER
#   4) DDQN-RASPBERry
...
#   ./run_apex_test_pong.sh -m pber        # run PBER only (APEX + DDQN)
#   ./run_apex_test_pong.sh -m raspberry   # run RASPBERry only (APEX + DDQN)
#
################################################################################
...
APEX_PBER_CONFIG="configs/apex_pber_atari.yml"
APEX_RASP_CONFIG="configs/apex_raspberry_atari.yml"
DDQN_PBER_CONFIG="configs/ddqn_pber_atari.yml"
DDQN_RASP_CONFIG="configs/ddqn_raspberry_atari.yml"

echo "================================================================================"
echo "APEX/DDQN test (Pong) | env: ${ENV_NAME}"
echo "Target GPUs: ${GPU_IDS[*]}"
case "${MODE}" in
    both) MODE_DESC="PBER (APEX+DDQN) -> RASPBERry (APEX+DDQN) per GPU" ;;
    pber) MODE_DESC="PBER only (APEX + DDQN)" ;;
    raspberry) MODE_DESC="RASPBERry only (APEX + DDQN)" ;;
esac
echo "Mode: ${MODE_DESC}"
...
for idx in "${!GPU_IDS[@]}"; do
    gpu="${GPU_IDS[$idx]}"
    echo "GPU ${gpu}: Pong APEX/DDQN test group $((idx + 1))"
    ...
    if [ "${MODE}" = "both" ] || [ "${MODE}" = "pber" ]; then
        ...
        python runner/run_apex_pber_algo.py --config ${APEX_PBER_CONFIG} --env ${ENV_NAME} --gpu ${gpu} \
            > ${SCRIPT_LOG_DIR}/apex_pber_${log_suffix}.log 2>&1 &
        ALL_PIDS+=($!)
        ALL_NAMES+=("GPU${gpu}-APEX-PBER")
        if [ ${task_idx} -lt ${task_total} ]; then
            sleep ${LAUNCH_DELAY_SAME_GPU}
        fi
        task_idx=$((task_idx + 1))
        echo "  [${task_idx}/${task_total}] DDQN-PBER (log: ${SCRIPT_LOG_DIR}/ddqn_pber_${log_suffix}.log)"
        python runner/run_ddqn_pber_algo.py --config ${DDQN_PBER_CONFIG} --env ${ENV_NAME} --gpu ${gpu} \
            > ${SCRIPT_LOG_DIR}/ddqn_pber_${log_suffix}.log 2>&1 &
        ALL_PIDS+=($!)
        ALL_NAMES+=("GPU${gpu}-DDQN-PBER")
        if [ ${task_idx} -lt ${task_total} ]; then
            sleep ${LAUNCH_DELAY_SAME_GPU}
        fi
    fi
    ...
    if [ "${MODE}" = "both" ] || [ "${MODE}" = "raspberry" ]; then
        ...
        python runner/run_apex_raspberry_algo.py --config ${APEX_RASP_CONFIG} --env ${ENV_NAME} --gpu ${gpu} \
            > ${SCRIPT_LOG_DIR}/apex_raspberry_${log_suffix}.log 2>&1 &
        ALL_PIDS+=($!)
        ALL_NAMES+=("GPU${gpu}-APEX-RASPBERry")
        if [ ${task_idx} -lt ${task_total} ]; then
            sleep ${LAUNCH_DELAY_SAME_GPU}
        fi
        task_idx=$((task_idx + 1))
        echo "  [${task_idx}/${task_total}] DDQN-RASPBERry (log: ${SCRIPT_LOG_DIR}/ddqn_raspberry_${log_suffix}.log)"
        python runner/run_ddqn_raspberry_algo.py --config ${DDQN_RASP_CONFIG} --env ${ENV_NAME} --gpu ${gpu} \
            > ${SCRIPT_LOG_DIR}/ddqn_raspberry_${log_suffix}.log 2>&1 &
        ALL_PIDS+=($!)
        ALL_NAMES+=("GPU${gpu}-DDQN-RASPBERry")
    fi
...
echo ""
echo "Submitted $(( ${#GPU_IDS[@]} * TASKS_PER_GPU )) APEX/DDQN test tasks"
