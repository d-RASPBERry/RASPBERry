...
echo "================================================================================"
echo "APEX ablation (Pong) | env: ${ENV_NAME}"
...
if [ "${GPU_ASSIGNMENT_MODE}" = "shared" ]; then
    for idx in "${!GPU_IDS[@]}"; do
        gpu="${GPU_IDS[$idx]}"
        echo "GPU ${gpu}: Pong APEX ablation group $((idx+1))"
    ...
    for ((group_idx=0; group_idx<GROUP_COUNT; group_idx++)); do
        ...
        log_suffix="pong_group$((group_idx + 1))_${TIMESTAMP}"

        echo "Group $((group_idx + 1)) Pong APEX ablation (exclusive)"
