...
################################################################################
# JR06 block-size ablation — group 2
# LavaCrossing seed1 m=64  +  Freeway seed0 m={8,16}
#
# Usage:
#   ./group2_lava_s1_freeway_s0.sh -n 0
#   ./group2_lava_s1_freeway_s0.sh -n 0,1,2 -m exclusive
################################################################################
...
CFG1="${CFG}/apex_raspberry_lavacrossing_s9n1_m64_seed1.yml"
CFG2="${CFG}/apex_raspberry_freeway_m8_seed0.yml"
CFG3="${CFG}/apex_raspberry_freeway_m16_seed0.yml"
...
echo "================================================================================"
echo "JR06 group2 — lava-s1-m64 / freeway-s0-m8 / freeway-s0-m16"
...
if [ "${GPU_ASSIGNMENT_MODE}" = "shared" ]; then
    for idx in "${!GPU_IDS[@]}"; do
        ...
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${gpu}-lava-s1-m64")
        echo "  [1/3] lava-s1-m64    (GPU ${gpu}) -> PID $!"
        sleep ${LAUNCH_DELAY_SAME_GPU}

        python ${RUNNER} --config "${CFG2}" --gpu "${gpu}" &
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${gpu}-freeway-s0-m8")
        echo "  [2/3] freeway-s0-m8  (GPU ${gpu}) -> PID $!"
        sleep ${LAUNCH_DELAY_SAME_GPU}

        python ${RUNNER} --config "${CFG3}" --gpu "${gpu}" &
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${gpu}-freeway-s0-m16")
        echo "  [3/3] freeway-s0-m16 (GPU ${gpu}) -> PID $!"
    ...
    for ((g=0; g<NUM_GPUS/3; g++)); do
        ...
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${g1}-lava-s1-m64")
        echo "  [lava-s1-m64]    GPU ${g1} -> PID $!"

        python ${RUNNER} --config "${CFG2}" --gpu "${g2}" &
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${g2}-freeway-s0-m8")
        echo "  [freeway-s0-m8]  GPU ${g2} -> PID $!"

        python ${RUNNER} --config "${CFG3}" --gpu "${g3}" &
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${g3}-freeway-s0-m16")
        echo "  [freeway-s0-m16] GPU ${g3} -> PID $!"
