...
################################################################################
# Block-size ablation - group 1
# LavaCrossingS9N1 seed1  m={8,16,32}
#
# Usage:
#   ./group1_lava_s1.sh -n 0
#   ./group1_lava_s1.sh -n 0,1,2 -m exclusive
################################################################################
...
CFG1="${CFG}/apex_raspberry_lavacrossing_s9n1_m8_seed1.yml"
CFG2="${CFG}/apex_raspberry_lavacrossing_s9n1_m16_seed1.yml"
CFG3="${CFG}/apex_raspberry_lavacrossing_s9n1_m32_seed1.yml"
...
echo "================================================================================"
echo "group1 - lava seed1 m={8,16,32}"
...
if [ "${GPU_ASSIGNMENT_MODE}" = "shared" ]; then
    ...
    for ((g=0; g<NUM_GPUS/3; g++)); do
        ...
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${g1}-m8")
        echo "  [m=8]  GPU ${g1} -> PID $!"

        python ${RUNNER} --config "${CFG2}" --gpu "${g2}" &
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${g2}-m16")
        echo "  [m=16] GPU ${g2} -> PID $!"

        python ${RUNNER} --config "${CFG3}" --gpu "${g3}" &
        ALL_PIDS+=($!); ALL_NAMES+=("GPU${g3}-m32")
        echo "  [m=32] GPU ${g3} -> PID $!"
