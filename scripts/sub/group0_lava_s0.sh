...
#   ./group0_lava_s0.sh -n 0,1,2 -m exclusive  # 1 experiment per GPU
################################################################################
...
while getopts "n:m:h" opt; do
    case $opt in
        ...
        h)
            echo "Usage: $0 [-n GPU_IDS] [-m shared|exclusive] [-h]"
            exit 0
            ;;
...
if [ "${GPU_ASSIGNMENT_MODE}" = "exclusive" ]; then
    if (( NUM_GPUS % 3 != 0 )); then
        echo "Error: exclusive mode requires GPU count multiple of 3 (got ${NUM_GPUS})" >&2
        exit 1
    fi
fi
...
CFG1="${CFG}/apex_raspberry_lavacrossing_s9n1_m8_seed0.yml"
CFG2="${CFG}/apex_raspberry_lavacrossing_s9n1_m32_seed0.yml"
CFG3="${CFG}/apex_raspberry_lavacrossing_s9n1_m64_seed0.yml"
...
echo "================================================================================"
echo "group0 - lava seed0 m={8,32,64}"
