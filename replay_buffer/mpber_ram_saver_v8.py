...
def decompress_sample_batch(ma_batch: SampleBatch, compress_base: int = -1) -> SampleBatch:
    ...
    if compress_base == -1:
        ...
        # 用户指定的维度恢复
        if compress_base >= rank or rank <= 1:
            decompressed_obs = decompressed_obs_transposed
            decompressed_new_obs = decompressed_new_obs_transposed
            transpose_type = "no_transpose"
        else:
...
def compress_sample_batch_loop(samples: List) -> List:
    """并行压缩样本批次的循环函数"""
    results = []
    for each in samples:
        results.append(compress_sample_batch(each[0], each[1], each[2]))
    return results


def compress_sample_batch(sample_batch: SampleBatch, weight: float, compress_base: int = -1) -> tuple:
    """压缩样本批次
...
class ParallelPrioritizedBlockReplayBuffer(PrioritizedBlockReplayBuffer):
    ...
    def _encode_sample(self, idxes: List[int]) -> SampleBatch:
        ...
        # 小批次或禁用并行时，使用原始处理
        if not self.enable_parallel_nodes or len(samples) < self.parallel_threshold:
            decompressed_samples = []
            for compressed_sample in samples:
                decompressed = decompress_sample_batch(compressed_sample, self.compress_base)
                decompressed_samples.append(decompressed)
            ...
            # 使用并行node sample
            futures = [_parallel_node_sample.remote(sample, self.compress_base) for sample in samples]
