#!/usr/bin/env python3
"""Minimal training script for DDQN + PER.

Direct algorithm construction using DQNConfig,
suitable for quick experiments.
Mirrors the structure of run_ddqn_raspberry_algo.py for easy comparison.
"""
...
from utils import env_creator, ConfigLoader, resolve_logging_config, infer_env_type

# ====== Section: Constants ======
DEFAULT_CONFIG_PATH = str((ROOT / "configs/ddqn_per.yml").resolve())
RUNTIME_CONFIG = str((ROOT / "configs/runtime.yml").resolve())

# ====== Section: Algorithm Construction ======

def build_algorithm(env_id: str, env_short: str, config: dict) -> DQN:
    """Build DQN PER algorithm instance.
    ...
    return DQN(config=hyper, env=env_short)


# ====== Section: CLI / Main ======
def main() -> None:
    # ------ Subsection: CLI args ------
    parser = argparse.ArgumentParser(description="DDQN-PER training script")
    parser.add_argument(
        "--env",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Path to DDQN-PER config YAML (default: ddqn_per.yml)",
    )
