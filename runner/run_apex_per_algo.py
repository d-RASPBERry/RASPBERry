#!/usr/bin/env python3
"""Minimal training script for APEX-DQN + PER.

Direct algorithm construction using ApexDQNConfig,
suitable for quick experiments.
Mirrors the structure of run_apex_raspberry_algo.py for easy comparison.
...
# ====== Section: Constants ======
DEFAULT_CONFIG_PATH = str((ROOT / "configs/apex_per.yml").resolve())
...
def build_algorithm(env_id: str, env_short: str, config: dict) -> ApexDQN:
    ...
    Returns:
        Configured ApexDQN algorithm instance
    """
    ...
    return ApexDQN(config=hyper, env=env_short)


# ====== Section: CLI / Main ======
def main() -> None:
    # ------ Subsection: CLI args ------
    parser = argparse.ArgumentParser(description="APEX-DQN-PER training script")
    ...
    parser.add_argument(
        ...
        help="Path to APEX-PER config YAML (default: apex_per.yml)",
    )
