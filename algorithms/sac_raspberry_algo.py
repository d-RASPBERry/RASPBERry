"""SAC RASPBERry algorithm implementation.

Extends RLlib's SAC with block-level prioritized replay buffer support,
including priority updates and compression handling for RASPBERry buffer.
"""
...
def update_priorities_in_replay_buffer(
    ...
    for policy_id, info in train_results.items():
        ...
        prio_dict[policy_id] = (np.asarray(batch_indices), np.asarray(td_error))

    if prio_dict:
        replay_buffer.update_priorities(prio_dict)


class SACRaspberryAlgo(SAC):
    """SAC algorithm customized to work with the RASPBERry replay buffer."""
    ...
    def training_step(self) -> ResultDict:
        ...
        # Mirror DQN-style rollout/train balance via training_intensity
